MODULE outmod
  ...
  contains
    ...
    subroutine write_mat(outUnit,MATdata,MatName,rundata)
      integer,intent(inout)                 ::  outUnit  !number of the output unit
      real,dimension(:,:),intent(inout)     ::  MATdata
      character(len=*),intent(in)           ::  MatName
      type(RunParms),intent(in)             :: rundata
      ...
      write(outUnit,'(A)') MatName
      do i=1,rundata%i_max
        write(outUnit,format_str) MATdata(i,:)
      end do
    end subroutine
end module
